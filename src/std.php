...
if (!function_exists('assoc')) {
    function assoc($path, callable $f, $xs)
    {
...
if (!function_exists('fold')) {
    function fold(callable $f, $v, array $xs)
    {
        $x = $v;
...
if (!function_exists('reduce')) {
    function reduce(callable $f, array $xs)
    {
...
if (!function_exists('filter')) {
    function filter(callable $p, array $xs): array
    {
        return fold(
...
if (!function_exists('any')) {
    function any(callable $p, array $xs): bool
    {
        return fold(
...
if (!function_exists('all')) {
    function all(callable $p, array $xs): bool
    {
        return fold(
            static fn ($x, $v, $k) => $x && $p($v, $k),
            true,
...
if (!function_exists('map')) {
    function map(callable $f, array $xs): array
    {
        return fold(
            static function ($x, $y) use ($f) {
                $x[] = $f($y);
                return $x;
            },
            [],
...
if (!function_exists('uniq')) {
    function uniq($xs)
    {
        switch (true) {
            case isString($xs):
                return join('', array_values(array_unique(str_split($xs))));
            case isList($xs): return array_values(array_unique($xs));
